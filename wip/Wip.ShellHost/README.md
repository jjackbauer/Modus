...
|---|---|---|
| `config` | `config` | Prints deterministic effective configuration fields for repository, .wip roots, repo and user plugin paths, workflow default, policy, startup mode, and validation commands. |
...
1. Defaults:
   - `pluginsPath = <currentDirectory>/.wip/plugins`
   - `userPluginsPath = <userProfile>/.wip/plugins`
   - `workspaceRoot = <currentDirectory>`
   - `wipRoot = <currentDirectory>/.wip`
   - `sessionsPath = <currentDirectory>/.wip/sessions`
   - `worktreesPath = <currentDirectory>/.wip/worktrees`
...
- `repositoryPath: <startup current directory>`
- `policy: <effective policy id>`
- `configPath: <absolute .wip/config.json path>`
- `configSource: <defaults|disk>`
- `wipRoot: <effective .wip root>`
- `sessionsPath: <effective sessions path>`
- `worktreesPath: <effective worktrees path>`
- `pluginsPath: <effective plugin path after precedence resolution>`
- `userPluginsPath: <user plugin path under home>`
- `workspaceRoot: <effective workspace root>`
- `defaultWorkflowId: <effective workflow id>`
- `pluginStartupMode: <effective startup mode>`
