...
public sealed class ModusWipBridge : IModusWipBridge, IModusWipDebugChannel
{
    ...
    public ValueTask<int> LoadPluginsAsync(CancellationToken cancellationToken)
    {
        ...
        var pluginAssemblyPathById = new Dictionary<string, string>(StringComparer.Ordinal);

        for (var index = 0; index < _pluginDiscoveryPaths.Count; index++)
        {
            cancellationToken.ThrowIfCancellationRequested();

            var pluginDiscoveryPath = _pluginDiscoveryPaths[index];
            var precedence = index + 1;
            AddLoadDiagnostic(DiscoveryStage, $"Plugin discovery path [precedence={precedence}]: {pluginDiscoveryPath}");

            if (!Directory.Exists(pluginDiscoveryPath))
            {
                AddLoadDiagnostic(DiscoveryStage, $"Plugin path [precedence={precedence}] '{pluginDiscoveryPath}' does not exist.");
                continue;
            }
                         ...
                         .OrderBy(static path => path, StringComparer.Ordinal))
            {
                ...
                foreach (var pluginType in ResolvePluginTypes(assembly))
                {
                    ...
                    if (pluginAssemblyPathById.TryGetValue(plugin.PluginId.Value, out var selectedAssemblyPath))
                    {
                        AddLoadDiagnostic(
                            DiscoveryStage,
                            $"Skipping plugin '{plugin.PluginId.Value}' from lower-precedence path '{assemblyPath}'; selected path '{selectedAssemblyPath}'.");
                        continue;
                    }
                    ...
                    try
                    {
                        ...
                        pluginAssemblyPathById[plugin.PluginId.Value] = assemblyPath;
                    }
                    catch (Exception ex)
                    {
    ...
    private RunManifest CreateUnloadedRunManifest()
        => new(DateTimeOffset.UtcNow, Array.Empty<PluginManifestEntry>(), BuildWorkflowEntries());
    ...
    private static IReadOnlyList<string> NormalizePluginDiscoveryPaths(IReadOnlyList<string> pluginDiscoveryPaths)
    {
        ...
        foreach (var path in pluginDiscoveryPaths.Where(static path => !string.IsNullOrWhiteSpace(path)))
        {
