...
public sealed class WipBuilderExplicitRegistrationTests
{
    ...
    [Fact]
    public void AddModelProviderTModelProviderTRequestPayloadTResultPayload_GivenUniqueId_RegistersResolvableTypedCapability()
    {
        var services = new ServiceCollection();
        var builder = new WipBuilder(services);

        builder.AddModelProvider<DeterministicModelProvider, PromptPayload, CompletionPayload>(
            capabilityId: new CapabilityId("model.provider.single"),
            displayName: "Deterministic model provider");

        var provider = services.BuildServiceProvider();
        var capability = provider.GetService<DeterministicModelProvider>();
        var descriptor = Assert.Single(builder.CapabilityDescriptors);

        Assert.NotNull(capability);
        Assert.Equal(typeof(ModelProviderRequest<PromptPayload>), descriptor.RequestType);
        Assert.Equal(typeof(ModelProviderResponse<CompletionPayload>), descriptor.ResultType);
        Assert.Equal(typeof(DeterministicModelProvider), descriptor.CapabilityType);
        Assert.Equal(CapabilityKind.ModelProvider, descriptor.Kind);
        Assert.Equal("model.provider.single", descriptor.CapabilityId.Value);
    }

    [Fact]
    public void AddAgentTAgentTRequestTResult_GivenMetadataContractInput_StoresFullDescriptorMetadata()
    {
        var services = new ServiceCollection();
        var builder = new WipBuilder(services);

        builder.AddAgent<PlanAgent, PlanRequest, PlanResult>(
            capabilityId: new CapabilityId("agent.plan.metadata"),
            displayName: "Plan metadata agent",
            version: new CapabilityVersion("2.4.0"),
            requiredPermissions:
            [
                new CapabilityPermission("workspace.read"),
                new CapabilityPermission("artifact.write")
            ],
            pluginOrigin: new PluginOrigin("plugin.plan", new CapabilityVersion("3.0.0"), "Plugin.Plan"));

        var descriptor = Assert.Single(builder.CapabilityDescriptors);

        Assert.Equal("agent.plan.metadata", descriptor.CapabilityId.Value);
        Assert.Equal("Plan metadata agent", descriptor.DisplayName);
        Assert.Equal(CapabilityKind.Agent, descriptor.Kind);
        Assert.Equal(typeof(PlanRequest), descriptor.RequestType);
        Assert.Equal(typeof(PlanResult), descriptor.ResultType);
        Assert.Equal(new CapabilityVersion("2.4.0"), descriptor.Version);
        Assert.Equal(
            new[] { new CapabilityPermission("workspace.read"), new CapabilityPermission("artifact.write") },
            descriptor.RequiredPermissions);
        Assert.Equal(new PluginOrigin("plugin.plan", new CapabilityVersion("3.0.0"), "Plugin.Plan"), descriptor.PluginOrigin);
    }

    [Fact]
    public void AddToolTToolTRequestTResult_GivenNoOptionalMetadata_UsesDeterministicDescriptorDefaults()
    {
        var services = new ServiceCollection();
        var builder = new WipBuilder(services);

        builder.AddTool<EchoTool, ToolRequest, ToolResult>(
            capabilityId: new CapabilityId("tool.echo.defaults"),
            displayName: "Echo defaults tool");

        var descriptor = Assert.Single(builder.CapabilityDescriptors);

        Assert.Equal("tool.echo.defaults", descriptor.CapabilityId.Value);
        Assert.Equal(new CapabilityVersion("1.0.0"), descriptor.Version);
        Assert.Empty(descriptor.RequiredPermissions);
        Assert.Null(descriptor.PluginOrigin);
    }
    ...
    [Fact]
    public void AddModelProvider_GivenDuplicateCapabilityId_RejectsRegistrationUnlessReplaceEnabled()
    {
        var services = new ServiceCollection();
        var builder = new WipBuilder(services);

        builder.AddModelProvider<DeterministicModelProvider, PromptPayload, CompletionPayload>(
            capabilityId: new CapabilityId("model.provider.duplicate"),
            displayName: "Deterministic model provider");

        var exception = Assert.Throws<InvalidOperationException>(() =>
            builder.AddModelProvider<ReplacementModelProvider, PromptPayload, ReplacementCompletionPayload>(
                capabilityId: new CapabilityId("model.provider.duplicate"),
                displayName: "Replacement model provider"));

        Assert.Contains("model.provider.duplicate", exception.Message, StringComparison.Ordinal);
    }

    [Fact]
    public void AddModelProvider_GivenDuplicateCapabilityIdAndReplaceEnabled_ReplacesExistingRegistrationDeterministically()
    {
        var services = new ServiceCollection();
        var builder = new WipBuilder(services)
            .EnableCapabilityReplacement();
        ...
        builder.AddModelProvider<DeterministicModelProvider, PromptPayload, CompletionPayload>(
            capabilityId: new CapabilityId("model.provider.replace"),
            displayName: "Deterministic model provider");
        ...
        var descriptor = Assert.Single(builder.CapabilityDescriptors);
        var originalService = provider.GetService<DeterministicModelProvider>();
        var replacementService = provider.GetService<ReplacementModelProvider>();

        Assert.Equal("model.provider.replace", descriptor.CapabilityId.Value);
        Assert.Equal(CapabilityKind.ModelProvider, descriptor.Kind);
        Assert.Equal(typeof(ReplacementModelProvider), descriptor.CapabilityType);
        Assert.Equal(typeof(ModelProviderRequest<PromptPayload>), descriptor.RequestType);
        Assert.Equal(typeof(ModelProviderResponse<ReplacementCompletionPayload>), descriptor.ResultType);
    ...
    private sealed record PromptPayload(string Prompt);

    private sealed record CompletionPayload(string Completion);

    private sealed record ReplacementCompletionPayload(string Completion);

    private sealed class DeterministicModelProvider : IModelProvider<PromptPayload, CompletionPayload>
    {
        public ValueTask<ModelProviderResponse<CompletionPayload>> ExecuteAsync(
            ModelProviderRequest<PromptPayload> request,
            CapabilityContext context,
            CancellationToken cancellationToken)
            => ValueTask.FromResult(new ModelProviderResponse<CompletionPayload>(
                new CompletionPayload(request.Payload.Prompt),
                providerId: "deterministic",
                modelId: request.ModelId,
                correlationId: request.CorrelationId));
    }

    private sealed class ReplacementModelProvider : IModelProvider<PromptPayload, ReplacementCompletionPayload>
    {
        public ValueTask<ModelProviderResponse<ReplacementCompletionPayload>> ExecuteAsync(
            ModelProviderRequest<PromptPayload> request,
            CapabilityContext context,
            CancellationToken cancellationToken)
            => ValueTask.FromResult(new ModelProviderResponse<ReplacementCompletionPayload>(
                new ReplacementCompletionPayload($"replacement:{request.Payload.Prompt}"),
