...
public sealed class BonesLearningLoopReliabilityBehaviorProofComplianceGateTests
{
    private const string ChecklistItem = BonesLearningLoopReliabilityRequirementsChecklistItems.BehaviorProofPolicy;
    ...
    [
        ...
        new(
            "WIP/Wip.Bones.ModelProviders.DeepSeek.Tests",
    ...
    public async Task BehaviorProofCompliance_GivenWipBonesLearningLoopReliabilityChecklist_RequiresExecutableRuntimeProofForEachItem()
    {
        ...
        var report = WipBonesLearningLoopReliabilityBehaviorProofVerifier.Evaluate(requirements, discoveredTestsUniqueByName);

        var nonCompliant = report.PlannedIntegrationTests
            .Where(static entry => !string.Equals(
                entry.Name,
                WipBonesLearningLoopReliabilityBehaviorProofVerifier.MetadataOnlyRejectionTestName,
        ...
        var plannedExecutions = IntegrityRemediationBehaviorProofVerifier.BuildPlannedExecutions(
            ...
            BehaviorProofTestProjects,
            repositoryRoot,
            currentExecutingTestName: nameof(BehaviorProofCompliance_GivenWipBonesLearningLoopReliabilityChecklist_RequiresExecutableRuntimeProofForEachItem));
        ...
        Assert.True(
            ...
            $"Every mapped planned integration test must execute successfully from the Wip.Bones.LearningLoopReliability compliance gate.{Environment.NewLine}{string.Join(Environment.NewLine + Environment.NewLine, failedExecutions)}");
    }
...
""";
        ...
        Assert.Equal(WipBonesLearningLoopReliabilityBehaviorProofVerifier.MetadataOnlyRejectionTestName, failedEntry);
    }
    ...
    private static string GetRequirementsDocumentPath()
        => Path.Combine(
            ...
            WipBonesLearningLoopReliabilityBehaviorProofVerifier.RequirementsRelativePath.Replace('/', Path.DirectorySeparatorChar));
