...
{
    private const string ChecklistItem = BonesScriptStrategiesRequirementsChecklistItems.BehaviorProofPolicy;
    ...
    public async Task BehaviorProofCompliance_GivenScriptStrategiesRequirements_RequiresExecutableRuntimeProofForEachItem()
    {
        ...
        var report = WipBonesScriptStrategiesBehaviorProofVerifier.Evaluate(requirements, discoveredTestsUniqueByName);

        var nonCompliant = report.PlannedIntegrationTests
            .Where(static entry => !string.Equals(
                entry.Name,
                WipBonesScriptStrategiesBehaviorProofVerifier.MetadataOnlyRejectionTestName,
        ...
        var plannedExecutions = IntegrityRemediationBehaviorProofVerifier.BuildPlannedExecutions(
            ...
            BehaviorProofTestProjects,
            repositoryRoot,
            currentExecutingTestName: nameof(BehaviorProofCompliance_GivenScriptStrategiesRequirements_RequiresExecutableRuntimeProofForEachItem));
        ...
        Assert.True(
            ...
            $"Every mapped planned integration test must execute successfully from the Wip.Bones.ScriptStrategies compliance gate.{Environment.NewLine}{string.Join(Environment.NewLine + Environment.NewLine, failedExecutions)}");
    }
...
""";
        ...
        Assert.Equal(WipBonesScriptStrategiesBehaviorProofVerifier.MetadataOnlyRejectionTestName, failedEntry);
    }
    ...
    private static string GetRequirementsDocumentPath()
        => Path.Combine(
            ...
            WipBonesScriptStrategiesBehaviorProofVerifier.RequirementsRelativePath.Replace('/', Path.DirectorySeparatorChar));
