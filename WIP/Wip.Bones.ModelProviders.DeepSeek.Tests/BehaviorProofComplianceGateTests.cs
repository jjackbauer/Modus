...
using Wip.Bones.ModelProviders.DeepSeek.Tests.Compliance;
...
{
    private const string ChecklistItem = BonesDeepSeekRequirementsChecklistItems.BehaviorProofPolicy;
    ...
    public async Task BehaviorProofCompliance_GivenWipBonesDeepSeekChecklistItems_RequiresExecutableRuntimeProofForEachItem()
    {
        ...
        var report = WipBonesDeepSeekBehaviorProofVerifier.Evaluate(requirements, discoveredTestsUniqueByName);

        var nonCompliant = report.PlannedIntegrationTests
            .Where(static entry => !string.Equals(
                entry.Name,
                WipBonesDeepSeekBehaviorProofVerifier.MetadataOnlyRejectionTestName,
        ...
        var plannedExecutions = IntegrityRemediationBehaviorProofVerifier.BuildPlannedExecutions(
            ...
            BehaviorProofTestProjects,
            repositoryRoot,
            currentExecutingTestName: nameof(BehaviorProofCompliance_GivenWipBonesDeepSeekChecklistItems_RequiresExecutableRuntimeProofForEachItem));
        ...
        Assert.True(
            ...
            $"Every mapped planned integration test must execute successfully from the Wip.Bones.DeepSeek compliance gate.{Environment.NewLine}{string.Join(Environment.NewLine + Environment.NewLine, failedExecutions)}");
    }
...
""";
        ...
        Assert.Equal(WipBonesDeepSeekBehaviorProofVerifier.MetadataOnlyRejectionTestName, failedEntry);
    }
    ...
    private static string GetRequirementsDocumentPath()
        => Path.Combine(
            FindRepositoryRoot(),
            WipBonesDeepSeekBehaviorProofVerifier.RequirementsRelativePath.Replace('/', Path.DirectorySeparatorChar));
