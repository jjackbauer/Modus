...
public sealed class ShellCommandTool : ITool<ShellCommandRequest, ShellCommandResult>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var blockedReason = PolicyViolationPayload.Create(
                    blockedAction: request.Command,
                    blockingPolicy: _policy.PolicyId.Value,
                    nextStepGuidance: "Remove dangerous command tokens from the requested action, then retry with a safe command.",
                    reason: DangerousCommandDenylist.BuildToolBlockedReason(dangerousMatch))
        ...
        {
            var blockedReason = PolicyViolationPayload.EnsureDeterministicPayload(
                blockedAction: request.Command,
                blockingPolicy: _policy.PolicyId.Value,
                nextStepGuidance: "Adjust the command or workflow state so it satisfies policy requirements, then retry.",
                reasonOrPayload: policyDecision.Reason);
