...
| AddWipCapabilities(Action<WipBuilderOptions>) | Creates a builder with explicit external-composition options, including duplicate capability behavior policy. | Configure WipBuilderOptions.DuplicateCapabilityBehavior to Reject (default) or ReplaceExisting. |
...
When replacement mode is enabled via EnableCapabilityReplacement() or configured as DuplicateCapabilityBehavior.ReplaceExisting through AddWipCapabilities(Action<WipBuilderOptions>), the second registration replaces the first registration for that capability ID:
