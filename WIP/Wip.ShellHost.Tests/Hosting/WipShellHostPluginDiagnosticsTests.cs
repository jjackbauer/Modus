...
public sealed class WipShellHostPluginDiagnosticsTests : IDisposable
{
    ...
    public async Task PluginsLoadCommand_GivenPluginsNotLoaded_ExpectedBridgeLoadInvokedAndManifestPopulated()
    {
        ...
        using var input = new StringReader("plugins\nplugins load\nplugins\nexit\n");
        using var output = new StringWriter();
        var loop = new WipShellCommandLoop(CreateOrchestrator(), input, output, diagnosticsBridge: bridge);
    ...
    public async Task WorkflowsCommand_GivenConcurrentPluginLogging_ExpectedWorkflowDiagnosticsRemainDeterministic()
    {
        ...
        var builder = new WipBuilder(new Microsoft.Extensions.DependencyInjection.ServiceCollection())
            .AddWorkflow<RuntimeAlphaWorkflow, RuntimeAlphaRequest, RuntimeAlphaResult>(
                new WorkflowId("workflow.runtime.alpha"),
                "Runtime Alpha Workflow");
        var loop = new WipShellCommandLoop(CreateOrchestrator(), input, output, builder, diagnosticsBridge: bridge);
    ...
    public async Task WorkflowsCommand_GivenExplicitLoadAndUnload_ExpectedWorkflowDiagnosticsTrackBridgeRuntimeState()
    {
        ...
        Assert.Equal(0, exitCode);
        Assert.Equal(3, CountOccurrences(rendered, "Registered workflows:"));
        Assert.Equal(0, CountOccurrences(rendered, "No workflows are currently registered."));
        Assert.Contains("Registered workflows:", rendered, StringComparison.Ordinal);
        Assert.Contains("workflow.runtime.alpha [Runtime Alpha Workflow]", rendered, StringComparison.Ordinal);
    }
    ...
    public async Task GetRunManifest_GivenStopAfterSuccessfulLoad_ExpectedPluginsAndWorkflowsClearedFromRuntimeSnapshot()
    {
        ...
        Assert.NotEmpty(manifestAfterStop.Workflows);
    }
    ...
    private sealed class RuntimeAlphaWorkflow : IWorkflow<RuntimeAlphaRequest, RuntimeAlphaResult>
    {
        public WorkflowId WorkflowId => new("workflow.runtime.alpha");

        public ValueTask<RuntimeAlphaResult> ExecuteAsync(
            RuntimeAlphaRequest request,
            WorkflowContext context,
            CancellationToken cancellationToken)
            => ValueTask.FromResult(new RuntimeAlphaResult(request.Value));
    }
