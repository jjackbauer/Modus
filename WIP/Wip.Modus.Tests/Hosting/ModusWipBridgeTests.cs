...
{
    private const string SampleExternalTypedPluginChecklistItem = "Add and verify sample external typed plugin package path proving typed capability descriptors, workflow visibility, and ambiguous inference failure isolation [depends on external plugin loading proof]";

    [Fact]
    [Trait("ChecklistItem", SampleExternalTypedPluginChecklistItem)]
    public void ExternalPluginBuild_GivenConsumerProjectUsesWipBuilderApis_RegistersTypedAgentToolValidatorAndWorkflowWithoutShellHostLogic()
    {
        ...
        var toolDescriptor = Assert.Single(builder.CapabilityDescriptors, static descriptor => descriptor.CapabilityId.Value == "todoapp.tool.draft");
        ...
        Assert.Equal(typeof(TodoDraftRequest), toolDescriptor.RequestType);
        Assert.Equal(typeof(TodoDraftResult), toolDescriptor.ResultType);
        ...
        Assert.Contains(services, static descriptor => descriptor.ServiceType == typeof(TodoDraftTool));
    ...
    [Fact]
    [Trait("ChecklistItem", SampleExternalTypedPluginChecklistItem)]
    public async Task ModusDiscovery_GivenExternalPluginAssemblyAndBuilderWorkflowRegistrations_ListsTypedCapabilitiesAndWorkflowWithoutShellRegistrationPath()
    {
        IServiceCollection services = new TestServiceCollection();
        var builder = services.AddWipCapabilities();
        builder.AddTodoAppWipAgents();
        ...
        try
        {
            ...
            Assert.Equal(new[] { "todoapp.agent.plan", "todoapp.tool.draft", "todoapp.validator.result" }, plugin.Capabilities);
