...
{
    private const string StaleReviewRecoveryGuidance = "Run 'review' to regenerate review evidence for the current diff hash, then retry 'approve'.";
    ...
    public ApprovalToken Create(ApprovalTokenRequest request)
    {
        ...
        if (request.ReviewReport.IsStale)
        {
            ...
            throw new InvalidOperationException($"{staleReason} {StaleReviewRecoveryGuidance}");
        }

        if (!string.Equals(request.DiffHash, request.ReviewReport.ReviewedDiffHash, StringComparison.Ordinal))
        {
            throw new InvalidOperationException(
                $"Approval token creation rejected: reviewed diff hash does not match the current approval candidate diff hash. {StaleReviewRecoveryGuidance}");
        }
