...
public sealed class WipRuntimeReviewGenerator
{
    ...
    private static string BuildMarkdown(ReviewRequest request, ReviewStaleness staleness, DateTimeOffset producedAtUtc)
    {
        ...
        lines.AppendLine($"Validation artifact id: {request.Validation.ArtifactId ?? "(missing)"}");
        lines.AppendLine($"Validation artifact path: {request.Validation.ArtifactPath ?? "(missing)"}");
