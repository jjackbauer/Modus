...
public sealed class TypedCapabilityContractsTests
{
    ...
    public void CapabilityDescriptor_GivenFirstClassCapabilityKind_PreservesKindInTypedDescriptor(CapabilityKind kind)
    {
        var descriptor = CapabilityDescriptor.For<TestAgent, CreateTaskRequest, CreateTaskResult>(
            capabilityId: new CapabilityId($"kind.{kind.ToString().ToLowerInvariant()}"),
