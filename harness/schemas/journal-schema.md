---
kind: schema
description: Canonical journal entry templates, field definitions, ID format, and confidence levels for exploration journals.
---

# Journal Schema

> **Usage**: Include this file in any prompt via `journal-schema.md`.
> This file is a reference-only document â€” it is not a runnable prompt.
>
> **Purpose**: Defines the canonical entry templates, field definitions, ID format, and
> confidence levels for all journal files under `harness/archive/journals/`. Every skill that reads
...
- **`{TYPE}`** â€” uppercase entry type prefix (see Â§2 for the full list).
- **`{NNN}`** â€” zero-padded three-digit sequence number, starting at `001` and incrementing per type.
...
|---|---|---|---|
| `OBS` | Observation | `harness/archive/journals/observations.md` | Grounded factual finding from source code |
| `HYP` | Hypothesis | `harness/archive/journals/hypotheses.md` | Testable claim derived from observations |
| `VAL` | Validation | `harness/archive/journals/validations.md` | Falsification or confirmation of a hypothesis |
| `DEC` | Decision | `harness/archive/journals/decisions.md` | Architectural or implementation decision with rationale |
| `TODO` | Todo | `harness/archive/journals/todos.md` | Actionable exploration or implementation task |
| `RISK` | Risk | `harness/archive/journals/risks.md` | Identified risk with likelihood and impact |
| `OQ` | Open Question | `harness/archive/journals/open-questions.md` | Unresolved question requiring further investigation |
| `ART` | Artifact Index | `harness/archive/journals/artifact-index.md` | Registry of generated downstream artifacts |
...
| Fact | Yes | A concrete, verifiable statement about what the code does. Must be grounded in read source â€” no inferences. |
| Implications | Yes | What this observation means for architecture, migration, or system understanding. |
| Confidence | Yes | One of: High, Medium, Low (see Â§3). |
| Related | No | Cross-references to other journal entries. Comma-separated IDs. |

---

### 4.2 HYP â€” Hypothesis
...
- **Falsification strategy**: {Specific steps to disprove this claim â€” what to look for and where}
...
| Status | Yes | Lifecycle state â€” see Â§5 below. |
| Confidence | Yes | One of: High, Medium, Low (see Â§3). |

**Status lifecycle**:

```
Proposed â†’ Under review â†’ Supported
                        â†’ Falsified
                        â†’ Superseded
```
...
### 4.3 VAL â€” Validation

```markdown
### VAL-{NNN}: {Short title}

- **Target HYP**: {HYP-NNN being validated}
- **Method**: {Description of the validation approach â€” which tactics were used}
...
- **Rationale**: {Why â€” with links to supporting evidence (OBS, HYP, VAL IDs)}
...
| Likelihood | Yes | One of: High, Medium, Low â€” probability of the risk materialising. |
| Impact | Yes | One of: High, Medium, Low â€” severity if the risk materialises. |
...
| Confidence | Yes | One of: High, Medium, Low (see Â§3). Reflects the overall trustworthiness of the artifact. |
...
- Updating the **Status** field of a HYP entry (e.g., Proposed â†’ Supported).
- Updating the **Status** field of a TODO entry (e.g., Open â†’ Done).
...
> Append-only journal. See `harness/schemas/journal-schema.md` for entry format.
