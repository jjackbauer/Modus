...
kind: skill
mode: agent
description: Skeptic/Falsifier role â€” attempt to falsify HYP entries or artifact claims, record VAL entries, and update HYP status.
composes:
  - ../schemas/journal-schema.md
  - falsify-claims.md
---

# Skill: Journal â€” Validate
...
You operate as a **Skeptic**. Your job is to try to break each hypothesis â€” not confirm it.
...
### Step 1 â€” Read the target HYP entries

Read `harness/archive/journals/hypotheses.md` and extract each specified HYP in full.
Update their status from `Proposed` â†’ `Under review` in `hypotheses.md`.

### Step 2 â€” Delegate to Falsify Claims skill

Format all HYP claims as a numbered list and invoke `falsify-claims`:
...
### Step 3 â€” Apply additional tactics
...
**Minimum requirement**: â‰¥ 2 distinct tactics per HYP entry. A validation that applies only one
tactic is incomplete and must not produce a final VAL entry.

### Step 4 â€” Classify each result

- **Supported**: The falsification attempt failed â€” no counter-evidence found and the claim holds
  across all examined code paths. Assign High confidence only if â‰¥ 3 tactics were tried and all failed.
...
### Step 5 â€” Record VAL entries

For each HYP, append a VAL entry to `harness/archive/journals/validations.md`:
...
### Step 6 â€” Update HYP status

In `harness/archive/journals/hypotheses.md`, update each target HYP:

- **Supported** â†’ set `Status: Supported`; upgrade confidence if warranted.
- **Falsified** â†’ set `Status: Falsified`; append counter-evidence OBS reference to Related.
  Create a new OBS entry documenting the counter-evidence (append to `observations.md`).
- **Unresolved** â†’ leave `Status: Under review`; add OQ entry for the gap.

### Step 7 â€” Report summary
...
**Do not** promote Supported findings to facts without going through `journal-distill`.