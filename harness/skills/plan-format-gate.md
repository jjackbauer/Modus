...
kind: skill
mode: agent
description: Validate a plan document's structure against the meta-structural conventions established in harness requirements documents.
composes:
  - falsify-claims.md
---
...
## Purpose
Given a plan document (migration requirements, parallelization audit, or generic), validate its Markdown structure against the formatting conventions used in `harness/requirements/`. Return a pass/fail verdict with a concrete fix-list when violations are found. This skill checks **structure only** â€” content correctness is the responsibility of the Falsify Claims skill.
...
### Rule 4 â€” Checklists with dependency tags

- Every `- [ ]` or `- [x]` item that describes a dependency or prerequisite must include a **bracketed tag** (e.g., `[prerequisite for many others]`, `[depends on X]`, `[mandatory â€” reason]`).
...
- This rule is lenient for items that already carry sufficient context via an inline description or parenthesised interface note (e.g., `- [ ] \`Add(Integer)\` â†’ \`operator+\` (\`IAdditionOperators<T,T,T>\`)`).

### Rule 5 â€” Mermaid diagrams (conditional)
...
### Rule 7 â€” Closing verification line
...
### Rule 8 â€” Numbered test plan items (conditional)
...
|---|---|---|---|
| 1 | Heading structure & separators | âœ… Pass | â€” |
| 2 | Scope statement under H1 | âœ… Pass | â€” |
| 3 | Pipe tables present | âœ… Pass | â€” |
| 4 | Checklists with dependency tags | âŒ Fail | 2 bare checklist items at lines 45, 52 |
| 5 | Mermaid diagrams | â­ï¸ Skipped | No triggering heading found |
| 6 | Verification gate | âœ… Pass | â€” |
| 7 | Closing verification line | âŒ Fail | Missing italicised "Zero Falsified" in last 5 lines |
| 8 | Numbered test plan items | âœ… Pass | â€” |

Use these status indicators:
- **âœ… Pass** â€” rule satisfied
- **âŒ Fail** â€” rule violated; list specific violations
- **â­ï¸ Skipped** â€” conditional rule not applicable to this document

Follow the table with a verdict line:

- If all non-skipped rules pass: `**PASS** â€” document conforms to plan format.`
- If any rule fails: `**FAIL** â€” N violation(s) found. Fix the items above and re-run this gate.`

## Loop Instruction (Self-Healing)

If the verdict is **FAIL**, the gate **itself** must:

1. Apply fixes directly to the document â€” edit the Markdown in-place to resolve each listed violation.
...
Do not ask the caller to fix violations manually â€” resolve them autonomously.
...
They are checked **in addition to** Rules 1â€“8 above (where applicable).
...
#### Rule J1 â€” Entry IDs follow `{TYPE}-{NNN}` format
...
#### Rule J2 â€” Required fields present per entry type
...
#### Rule J3 â€” HYP entries have a non-vague Falsification strategy
...
#### Rule J6 â€” VAL Result values are constrained
...
These rules validate a distilled knowledge document under `harness/archive/distilled/`.

#### Rule D1 â€” Standard header present
...
#### Rule D2 â€” All claims carry exactly one uncertainty marker

Every bullet point or sentence within the document body that makes a factual claim must begin with
exactly one of: `âœ…`, `âš ï¸`, or `â“`. Claims without a marker are flagged.
Claims with more than one marker on the same line are also flagged.

#### Rule D3 â€” Claims cite source entries

Every claim marked with âœ… or âš ï¸ must include at least one journal entry ID in parentheses
(e.g., `(OBS-001)` or `(OBS-003, VAL-005)`). Claims without parenthetical citations are flagged.

#### Rule D4 â€” `trusted-facts.md` contains only âœ… markers

If the document being validated is `trusted-facts.md`, any âš ï¸ or â“ markers found are flagged
as violations. That document is strictly âœ…-only.

#### Rule D5 â€” Overall Confidence matches marker distribution
...
|---|---|
| High | All claims are âœ…; zero âš ï¸ or â“ |
| Medium | Majority âœ… or âš ï¸; â“ are a minority |
| Low | Significant â“ present |
...
| # | Rule | Status | Violations |
|---|---|---|---|
| J1 | Entry ID format | âœ… Pass / âŒ Fail | â€” |
| J2 | Required fields per type | âœ… Pass / âŒ Fail | â€” |
| J3 | Non-vague falsification strategy | âœ… Pass / âŒ Fail | â€” |
| J4 | Confidence values | âœ… Pass / âŒ Fail | â€” |
| J5 | HYP status lifecycle | âœ… Pass / âŒ Fail | â€” |
| J6 | VAL result values | âœ… Pass / âŒ Fail | â€” |

Or for `"distilled"`:

| # | Rule | Status | Violations |
|---|---|---|---|
| D1 | Standard header present | âœ… Pass / âŒ Fail | â€” |
| D2 | All claims have one marker | âœ… Pass / âŒ Fail | â€” |
| D3 | Claims cite source entries | âœ… Pass / âŒ Fail | â€” |
| D4 | trusted-facts.md âœ…-only | âœ… Pass / â­ï¸ Skipped / âŒ Fail | â€” |
| D5 | Confidence matches markers | âœ… Pass / âŒ Fail | â€” |
